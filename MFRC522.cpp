/*
* MFRC522.cpp - Library to use IDF RFID MODULE KIT 13.56 MHZ WITH TAGS SPI W AND R BY COOQROBOT.
...
/////////////////////////////////////////////////////////////////////////////////////
// Functions for setting up the IDF
/////////////////////////////////////////////////////////////////////////////////////
...
void MFRC522::PCD_WriteRegister(	PCD_Register reg,	///< The register to write to. One of the PCD_Register enums.
    ...
    ESP_ERROR_CHECK(
        _spi->writeByAddress((reg & 0x7F), values, count)
    );
} // End PCD_WriteRegister()
...
uint8_t MFRC522::PCD_ReadRegister(	PCD_Register reg	///< The register to read from. One of the PCD_Register enums.
	...
	ESP_ERROR_CHECK(
		_spi->readByteByAddress((reg | 0x80), &value)
	);
...
void MFRC522::PCD_ReadRegister(	PCD_Register reg,	///< The register to read from. One of the PCD_Register enums.
    ...
    while (index < count) {
        ESP_ERROR_CHECK(
            // Read value and tell that we want to read the same address again.
            _spi->readByteByAddress((reg | 0x80), &values[index])
        );
...
void MFRC522::PCD_Init() {
	...
	if (_resetPowerDownPin != UNUSED_PIN) {
		...
		gpio_set_direction((gpio_num_t)_resetPowerDownPin, GPIO_MODE_INPUT);

		if (gpio_get_level((gpio_num_t)_resetPowerDownPin) == 0) {	// The MFRC522 chip is in power down mode.
			gpio_set_direction((gpio_num_t)_resetPowerDownPin, GPIO_MODE_OUTPUT); // Now set the resetPowerDownPin as digital output.
			gpio_set_level((gpio_num_t)_resetPowerDownPin, 0);		// Make shure we have a clean 0 state.
			vTaskDelay(1 / portTICK_PERIOD_MS); // 8.8.1 Reset timing requirements says about 100ns. Let us be generous: 2μsl
			gpio_set_level((gpio_num_t)_resetPowerDownPin, 1);		// Exit power down mode. This triggers a hard reset.
...
void MFRC522::PCD_SoftPowerUp(){
	...
	const uint64_t timeout = (esp_timer_get_time() / 1000) + 500;// create timer for timeout (just in case)

	while((esp_timer_get_time() / 1000)<=timeout){ // set timeout to 500 ms
...
/**
 * Returns a pointer to a status code name.
 *
 * @return const *
 */
...
void MFRC522::PCD_DumpVersionToSerial() {
	...
	ESP_LOGI(TAG, "Firmware Version: 0x%x", v);
	...
	if ((v == 0x00) || (v == 0xFF))
		ESP_LOGI(TAG, "WARNING: Communication failure, is the MFRC522 properly connected?");
...
void MFRC522::PICC_DumpToSerial(Uid *uid	///< Pointer to Uid struct returned from a successful PICC_Select().
	...
	switch (piccType) {
		...
		case PICC_TYPE_TNP3XXX:
			ESP_LOGI(TAG, "Dumping memory contents not implemented for that PICC type.");
			break;
...
void MFRC522::PICC_DumpDetailsToSerial(Uid *uid	///< Pointer to Uid struct returned from a successful PICC_Select().
	...
	for (uint8_t i = 0; i < uid->size; i++) {
		ESP_LOGI(TAG, "0x%x", uid->uidByte[i]);
	}
	ESP_LOGI(TAG, "");

	// SAK
	ESP_LOGI(TAG, "Card SAK: 0x%x", uid->sak);

	// (suggested) PICC type
	PICC_Type piccType = PICC_GetType(uid->sak);
	ESP_LOGI(TAG, "PICC type: %s", PICC_GetTypeName(piccType));;
...
void MFRC522::PICC_DumpMifareClassicSectorToSerial(Uid *uid,			///< Pointer to Uid struct returned from a successful PICC_Select().
	...
	for (int8_t blockOffset = no_of_blocks - 1; blockOffset >= 0; blockOffset--) {
		...
		if (isSectorTrailer) {
			...
			if (status != STATUS_OK) {
				ESP_LOGI(TAG, "PCD_Authenticate() failed: %s", GetStatusCodeName(status));
				return;
			}
		}
		// Read block
		...
		if (status != STATUS_OK) {
			ESP_LOGI(TAG, "MIFARE_Read() failed: %s", GetStatusCodeName(status));
			continue;
		}
		// Dump data
		for (uint8_t index = 0; index < 16; index++) {
			ESP_LOGI(TAG, "0x%x", buffer[index]);
		}
		...
		if (firstInGroup) {
			// Print access bits
			ESP_LOGI(TAG, " [ %d %d %d ]", (g[group] >> 2) & 1, (g[group] >> 1) & 1, (g[group] >> 0) & 1);
			if (invertedError) {
				ESP_LOGI(TAG, "Inverted access bits did not match!");
			}
		}

		if (group != 3 && (g[group] == 1 || g[group] == 6)) { // Not a sector trailer, a value block
			int32_t value = (int32_t(buffer[3])<<24) | (int32_t(buffer[2])<<16) | (int32_t(buffer[1])<<8) | int32_t(buffer[0]);
			ESP_LOGI(TAG, "Value=0x%x", value);
			ESP_LOGI(TAG, "Adr=0x%x", buffer[12]);
		}
		ESP_LOGI(TAG, "");
	}
...
bool MFRC522::MIFARE_OpenUidBackdoor(bool logErrors) {
	...
	if(status != STATUS_OK) {
		if(logErrors) {
			ESP_LOGI(TAG, "Card did not respond to 0x40 after HALT command. Are you sure it is a UID changeable one?");
			ESP_LOGI(TAG, "Error name: %s", GetStatusCodeName(status));
		}
		return false;
	}
	if (received != 1 || response[0] != 0x0A) {
		if (logErrors) {
			ESP_LOGI(TAG, "Got bad response on backdoor 0x40 command: %x (%d valid bits)", response[0], validBits);
		}
		return false;
	}
	...
	if(status != STATUS_OK) {
		if(logErrors) {
			ESP_LOGI(TAG, "Error in communication at command 0x43, after successfully executing 0x40");
			ESP_LOGI(TAG, "Error name: %s", GetStatusCodeName(status));
		}
		return false;
	}
	if (received != 1 || response[0] != 0x0A) {
		if (logErrors) {
			ESP_LOGI(TAG, "Got bad response on backdoor 0x43 command: %x (%d valid bits)", response[0], validBits);
		}
		return false;
	}
...
bool MFRC522::MIFARE_SetUid(uint8_t *newUid, uint8_t uidSize, bool logErrors) {
	...
	if (!newUid || !uidSize || uidSize > 15) {
		if (logErrors) {
			ESP_LOGI(TAG, "New UID buffer empty, size 0, or size > 15 given");
		}
		return false;
	}
...
//			  PICC_WakeupA(atqa_answer, &atqa_size);
			...
			if (!PICC_IsNewCardPresent() || !PICC_ReadCardSerial()) {
				ESP_LOGI(TAG, "No card was previously selected, and none are available. Failed to set UID.");
				return false;
			}
			...
			if (status != STATUS_OK) {
				...
				if (logErrors) {
					ESP_LOGI(TAG, "Failed to authenticate to card for reading, could not set UID: %s", GetStatusCodeName(status));
				}
				return false;
			}
		}
	...
	if (status != STATUS_OK) {
		if (logErrors) {
			ESP_LOGI(TAG, "MIFARE_Read() failed: %s", GetStatusCodeName(status));
			ESP_LOGI(TAG, "Are you sure your KEY A for sector 0 is 0xFFFFFFFFFFFF?");
		}
		return false;
	}
	...
	if (!MIFARE_OpenUidBackdoor(logErrors)) {
		if (logErrors) {
			ESP_LOGI(TAG, "Activating the UID backdoor failed.");
		}
		return false;
	}
	...
	if (status != STATUS_OK) {
		if (logErrors) {
			ESP_LOGI(TAG, "MIFARE_Write() failed: %s", GetStatusCodeName(status));
		}
		return false;
	}
